...
const FarmCards: React.FC = () => {
  ...
  const YFIGIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'YFiG',
  )

  const YFIGPrice =
  YFIGIndex >= 0 && stakedValue[YFIGIndex]
      ? stakedValue[YFIGIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const YFIG_PER_BLOCK = new BigNumber(60)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [YFIG, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'YFiG' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
